...
function isAudioOutput(output: unknown): { voice_url: string; text?: string } | null {
  if (output && typeof output === 'object' && 'voice_url' in output) {
    return output as { voice_url: string; text?: string };
  }
  return null;
}
  ...
  return (
    <div style={{ padding: '8px 16px', flex: 1, overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {logs.map((log, i) => {
          const audioOutput = isAudioOutput(log.output);
          ...
          return (
            ...
            >
              ...
              {/* Audio player for audio synth node */}
              {audioOutput && log.status === 'success' && (
                <div style={{ marginTop: 8 }}>
                  <AudioPlayer audioUrl={audioOutput.voice_url} text={audioOutput.text} />
                </div>
              )}

              {/* Regular output display (non-audio) */}
              {log.output != null && !audioOutput && (
                <div
                  style={{
